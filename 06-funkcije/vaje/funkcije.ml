...
let repeat x n = 
  let rec repeat_aux acc = function
    | 0 -> acc
    | m -> repeat_aux (x :: acc) (m - 1)
  in repeat_aux

let rec repeat x n = if n > 0 then x :: repeat x (n - 1) else []
...
let rec loop condition f x = 
  if condition x then loop condition f x else x
...
let apply_sequence f x n =
  let rec apply_aux acc x m = 
    if m < 0 then reverse acc else apply_aux (x :: acc) (f x) (m - 1)
  in apply_aux [] x n
